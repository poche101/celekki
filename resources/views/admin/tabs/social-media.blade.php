<div class="space-y-8 pb-12" x-data="{ deleteModal: false, deleteUrl: '' }">
    <div class="flex justify-between items-center">
        ...
        <div class="flex gap-3">
            <a href="{{ route('admin.social.export') }}"
                class="flex items-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg text-sm font-semibold text-slate-700 hover:bg-slate-50 transition-all">
    ...
    <div class="bg-white rounded-3xl border border-slate-200 shadow-sm overflow-hidden">
        <div class="border-b border-slate-100 bg-slate-50/50 px-8 py-4 flex justify-between items-center">
            ...
            <span class="text-xs text-slate-500">Showing {{ $viewers->firstItem() }}-{{ $viewers->lastItem() }} of
                {{ $viewers->total() }}</span>
        </div>
                            ...
                            <td class="px-8 py-5">
                                <div class="flex flex-col">
                                    ...
                                    <span class="text-[10px] text-slate-400 mt-1">
                                        ...
                                        {{ $viewer->created_at->format('h:i A') }}
                                    </span>
                                </div>
                            </td>
                            <td class="px-8 py-5 text-right">
                                <div x-data="{ open: false }" class="relative inline-block text-left">
                                    <button @click="open = !open" @click.away="open = false"
                                        class="p-2 hover:bg-white rounded-lg border border-transparent hover:border-slate-200 transition-all focus:outline-none">
                                    ...
                                    <div x-show="open"
                                        ...
                                        <div class="py-1">
                                            <button type="button"
                                                @click="deleteUrl = '{{ route('admin.viewers.destroy', $viewer->id) }}'; deleteModal = true; open = false"
    ...
    <template x-teleport="body">
        <div x-show="deleteModal" class="fixed inset-0 z-[100] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto" x-cloak>
            ...
            <div x-show="deleteModal"
                ...
                <div class="p-8 text-center">
                    ...
                    <h3 class="text-xl font-bold text-slate-900 mb-2">Confirm Deletion</h3>
                    <p class="text-slate-500 text-sm">Are you sure you want to delete this viewer record? This action cannot be undone and will remove all associated engagement history.</p>
                </div>

                <div class="flex border-t border-slate-100">
                    <button @click="deleteModal = false"
                            class="flex-1 px-6 py-4 text-sm font-bold text-slate-500 hover:bg-slate-50 border-r border-slate-100 transition-colors">
                        Cancel
                    </button>
                    ...
                    <form :action="deleteUrl" method="POST" class="flex-1">
                        ...
                        @method('DELETE')
                        <button type="submit"
                                class="w-full px-6 py-4 text-sm font-bold text-red-600 hover:bg-red-50 transition-colors">
                            Yes, Delete
                        </button>
                    </form>
                </div>
